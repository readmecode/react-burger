...
import { IFeedInfo } from "../../../utils/interfaces/interface";
import feedInfoStyle from "./feedinfo.module.css"

const FeedInfo: FC<IFeedInfo> = ({feedData, total, totalToday}) => {
  return (  
    <div className={feedInfoStyle.feedInfo}>
        ...
        <li className={feedInfoStyle.feedInfo__block}>
          <h4 className={feedInfoStyle.feedInfo__block__title}>Готовы:</h4>
          {
            ...
            feedData
              ...
              .map(el => (
                <p className={feedInfoStyle.feedInfo__num__order} key={el._id}>{el.number}</p>
            ))
          }
        </li>
        <li className={feedInfoStyle.feedInfo__block}>
      ...
      <span className={feedInfoStyle.feedInfo__total__container}>
        <p className={feedInfoStyle.feedInfo__uppertitle}>Выполнено за все время:</p>
        <h2 className={feedInfoStyle.feedInfo__total__title}>{total}</h2>
      </span>

      <span className={feedInfoStyle.feedInfo__total__container}>
        <p className={feedInfoStyle.feedInfo__uppertitle}>Выполнено за сегодня:</p>
